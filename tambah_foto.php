        ...
        if(isset($_REQUEST['submit'])){
            ...
            if($_REQUEST['no_agenda'] == "" || $_REQUEST['no_surat'] == "" || $_REQUEST['asal_surat'] == "" || $_REQUEST['isi'] == ""
                ...
                if(!preg_match("/^[0-9]*$/", $no_agenda)){
                    $_SESSION['no_agenda'] = 'harus diisi angka!';
                    echo '<script language="javascript">window.history.back();</script>';
                } else {

                    if(!preg_match("/^[a-zA-Z0-9.\/ -]*$/", $no_surat)){
                        $_SESSION['no_surat'] = 'hanya boleh mengandung karakter huruf, angka, spasi, titik(.), minus(-) dan garis miring(/)';
                        echo '<script language="javascript">window.history.back();</script>';
                    } else {

                        if(!preg_match("/^[a-zA-Z0-9.,() \/ -]*$/", $asal_surat)){
                            $_SESSION['asal_surat'] = 'hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-),kurung() dan garis miring(/)';
                            echo '<script language="javascript">window.history.back();</script>';
                        } else {

                            if(!preg_match("/^[a-zA-Z0-9.,_()%&@;:\/\r\n -]*$/", $isi)){
                                $_SESSION['isi'] = 'hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), kurung(), underscore(_), dan(&) persen(%) dan at(@)';
                                echo '<script language="javascript">window.history.back();</script>';
                            } else {

                                if(!preg_match("/^[a-zA-Z0-9.,\/ () & :;]*$/", $nkode)){
                                    $_SESSION['kode'] = 'hanya boleh mengandung karakter huruf, angka, spasi, titik(.) dan koma(,)';
                                    echo '<script language="javascript">window.history.back();</script>';
                                } else {

                                    if(!preg_match("/^[a-zA-Z0-9., -]*$/", $indeks)){
                                        $_SESSION['indeks'] = 'hanya boleh mengandung karakter huruf, angka, spasi, titik(.) dan koma(,) dan minus (-)';
                                        echo '<script language="javascript">window.history.back();</script>';
                                    } else {

                                        if(!preg_match("/^[0-9.-]*$/", $tgl_surat)){
                                            $_SESSION['tgl_surat'] = 'hanya boleh mengandung angka dan minus(-)';
                                            echo '<script language="javascript">window.history.back();</script>';
                                        } else {

                                            if(!preg_match("/^[a-zA-Z0-9.,()\/&;@%#\n\r-_!?'':]*$/", $keterangan)){
                                                $_SESSION['keterangan'] = 'hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), dan kurung()';
                                                echo '<script language="javascript">window.history.back();</script>';
                                            } else {

                                             //   $cek = mysqli_query($config, "SELECT * FROM tb_foto WHERE no_surat='$no_surat'");
                                                    ...
                                                    if($file != ""){
                                                        ...
                                                        if(in_array($eks, $ekstensi) == true){
                                                            if($ukuran < 20044070 ){
                                                                ...
                                                                $query = mysqli_query($config, "INSERT INTO tb_foto(no_agenda,no_surat,asal_surat,isi,kode,indeks,tgl_surat,
                                                        ...
                                                        $query = mysqli_query($config, "INSERT INTO tb_foto(no_agenda,no_surat,asal_surat,isi,kode,indeks,tgl_surat, tgl_diterima,file,keterangan,id_user)
                ...
                <div class="col s12">
                    <nav class="secondary-nav">
                        <div class="nav-wrapper blue darken-1">
                            <ul class="left">
                                <li class="waves-effect waves-light"><a href="?page=tf&act=add" class="judul"><i class="material-icons"></i> Tambah Data </a></li>
                            </ul>
                        </div>
                    </nav>
                </div>
            ...
            <div class="row jarak-form">
                ...
                <!-- Form START -->
                <form class="col s12" method="POST" action="?page=tf&act=add" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="input-field col s6">
                            ...
                            echo '<input id="no_agenda" type="number" class="validate" name="no_agenda" value="';
                                $sql = mysqli_query($config, "SELECT no_agenda FROM tb_foto");
                                $no_agenda = "1";
                        ...
                        <div class="input-field col s6">
                           <!-- <i class="material-icons prefix md-prefix">place</i> -->
                            <input id="asal_surat" type="text" class="validate" name="asal_surat" required>
                                ...
                                ?>
                            <label for="asal_surat">Jenis File</label>
                        </div>
                        <div class="input-field col s6">
                            ...
                            <div class="file-field input-field">
                                ...
                                <div class="file-path-wrapper">
                                    <input class="file-path validate" type="text" placeholder="Upload file">
                                        ...
                                        ?>
                                    <small class="red-text">*Format file yang diperbolehkan *.JPG, *.PNG, *.DOC, *.DOCX, *.PDF dan ukuran maksimal file  MB!</small>
                                </div>
                            </div>
                        </div>
                    </div>
