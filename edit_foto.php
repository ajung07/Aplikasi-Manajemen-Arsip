<?php
    ...
    if(empty($_SESSION['admin'])){
        ...
        if(isset($_REQUEST['submit'])){
            ...
            if($_REQUEST['no_agenda'] == "" || $_REQUEST['no_surat'] == "" || $_REQUEST['asal_surat'] == "" || $_REQUEST['isi'] == ""
                ...
                if(!preg_match("/^[0-9]*$/", $no_agenda)){
                    $_SESSION['eno_agenda'] = 'harus diisi angka!';
                    echo '<script language="javascript">window.history.back();</script>';
                } else {

                    if(!preg_match("/^[a-zA-Z0-9.\/ -]*$/", $no_surat)){
                        $_SESSION['eno_surat'] = 'hanya boleh mengandung karakter huruf, angka, spasi, titik(.), minus(-) dan garis miring(/)';
                        echo '<script language="javascript">window.history.back();</script>';
                    } else {

                        if(!preg_match("/^[a-zA-Z0-9.,() \/ -]*$/", $asal_surat)){
                            $_SESSION['easal_surat'] = 'hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-),kurung() dan garis miring(/)';
                            echo '<script language="javascript">window.history.back();</script>';
                        } else {

                            if(!preg_match("/^[a-zA-Z0-9.,_()%&@;:\/\r\n -]*$/", $isi)){
                                $_SESSION['eisi'] = 'hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), kurung(), underscore(_), dan(&) persen(%) dan at(@)';
                                echo '<script language="javascript">window.history.back();</script>';
                            } else {

                                if(!preg_match("/^[a-zA-Z0-9.,()&;\/@ :]*$/", $nkode)){
                                    $_SESSION['ekode'] = 'hanya boleh mengandung karakter huruf, angka, spasi, titik(.) dan koma(,)';
                                    echo '<script language="javascript">window.history.back();</script>';
                                } else {

                                    if(!preg_match("/^[a-zA-Z0-9., -]*$/", $indeks)){
                                        $_SESSION['eindeks'] = 'hanya boleh mengandung karakter huruf, angka, spasi, titik(.) dan koma(,) dan minus (-)';
                                        echo '<script language="javascript">window.history.back();</script>';
                                    } else {

                                        if(!preg_match("/^[0-9.-]*$/", $tgl_surat)){
                                            $_SESSION['etgl_surat'] = 'hanya boleh mengandung angka dan minus(-)';
                                            echo '<script language="javascript">window.history.back();</script>';
                                        } else {

                                            if(!preg_match("/^[a-zA-Z0-9.,()\/&;@%#\n\r-_!?:'']*$/", $keterangan)){
                                                $_SESSION['eketerangan'] = 'hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), dan kurung()';
                                                echo '<script language="javascript">window.history.back();</script>';
                                            } else {
                                            ...
                                            if($file != ""){
                                                ...
                                                if(in_array($eks, $ekstensi) == true){
                                                    if($ukuran < 20044070 ){
                                                        ...
                                                        $query = mysqli_query($config, "SELECT file FROM tb_foto WHERE id_surat='$id_surat'");
                                                        ...
                                                        if(!empty($file)){
                                                            ...
                                                            $query = mysqli_query($config, "UPDATE tb_foto SET no_agenda='$no_agenda',no_surat='$no_surat',asal_surat='$asal_surat',isi='$isi',kode='$nkode',indeks='$indeks',tgl_surat='$tgl_surat',file='$nfile',keterangan='$keterangan',id_user='$id_user' WHERE id_surat='$id_surat'");

                                                            if($query == true){
                                                                $_SESSION['succEdit'] = 'SUKSES! Data berhasil diupdate';
                                                                header("Location: ./admin.php?page=tf");
                                                                die();
                                                            } else {
                                                ...
                                                $query = mysqli_query($config, "UPDATE tb_foto SET no_agenda='$no_agenda',no_surat='$no_surat',asal_surat='$asal_surat',isi='$isi',kode='$nkode',indeks='$indeks',tgl_surat='$tgl_surat',keterangan='$keterangan',id_user='$id_user' WHERE id_surat='$id_surat'");
        ...
        $query = mysqli_query($config, "SELECT id_surat, no_agenda, no_surat, asal_surat, isi, kode, indeks, tgl_surat, file, keterangan, id_user FROM tb_foto WHERE id_surat='$id_surat'");
        ...
        if($_SESSION['id_user'] != $id_user AND $_SESSION['id_user'] != 1){
            echo '<script language="javascript">
                    ...
                    window.location.href="./admin.php?page=tf";
                  </script>';
        } else {?>
                ...
                <div class="col s12">
                    <nav class="secondary-nav">
                        <div class="nav-wrapper blue darken-1">
                            <ul class="left">
                                <li class="waves-effect waves-light"><a href="#" class="judul"><i class="material-icons"></i> Edit Foto</a></li>
                            </ul>
                        </div>
                    </nav>
                </div>
            ...
            <div class="row jarak-form">
                ...
                <!-- Form START -->
                <form class="col s12" method="POST" action="?page=tf&act=edit" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="input-field col s6">
                         <!--   <i class="material-icons prefix md-prefix">place</i> -->
                            <input id="asal_surat" type="text" class="validate" name="asal_surat" value="<?php echo $asal_surat ;?>" required>
                                ...
                                ?>
                           <!--  <label for="asal_surat">Jenis Foto</label> -->
                        </div>
                        <div class="input-field col s6">
                           <!-- <i class="material-icons prefix md-prefix">storage</i> -->
                            <input id="indeks" type="text" class="validate" name="indeks" value="<?php echo $indeks ;?>" required>
                                ...
                                ?>
                            <label for="indeks">Indeks Foto</label>
                        </div>
                        <div class="input-field col s6">
                        <!--    <i class="material-icons prefix md-prefix">looks_two</i> -->
                            <input id="no_surat" type="text" class="validate" name="no_surat" value="<?php echo $no_surat ;?>" required>
                                ...
                                ?>
                            <label for="no_surat">Katagori </label>
                        </div>
                        <div class="input-field col s6">
                       <!--     <i class="material-icons prefix md-prefix">date_range</i> -->
                            <input id="tgl_surat" type="text" name="tgl_surat" class="datepicker" value="<?php echo $tgl_surat ;?>" required>
                                ...
                                ?>
                            <label for="tgl_surat">Tanggal Foto</label>
                        </div>
                        <div class="input-field col s6">
                        ...
                        <!--    <i class="material-icons prefix md-prefix">featured_play_list</i> -->
                            <input id="keterangan" type="text" class="validate" name="keterangan" value="<?php echo $keterangan ;?>" required>
                                ...
                                ?>
                            <label for="keterangan">Ringkasan</label>
                        </div>
                        <div class="input-field col s6">
                    ...
                    <div class="row">
                        ...
                        <div class="col 6">
                            <a href="?page=tf" class="btn-large deep-orange waves-effect waves-light">BATAL <i class="material-icons">clear</i></a>
                        </div>
                    </div>
